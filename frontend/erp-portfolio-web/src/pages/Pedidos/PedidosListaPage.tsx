/**
 * =====================================================================
 * Arquivo....: PedidosListaPage.tsx
 * Versão.....: 1.0.0
 * Data.......: 21/09/2026
 * Descrição..: Tela de listagem de pedidos: filtros (busca por número ou nome do
 *              cliente e status) num painel que abre a partir do botão "Filtrar",
 *              tabela com paginação no servidor (mais recentes primeiro) e ações
 *              para abrir um pedido ou criar um novo. No celular, cada pedido vira
 *              um cartão.
 * ---------------------------------------------------------------------
 * Fontes.....: GET /api/pedidos?busca=&status=&pagina=&tamanhoPagina=
 *              (via useListaPedidos)
 * ---------------------------------------------------------------------
...
import { OPCOES_STATUS_PEDIDO, type PedidoFiltro, type PedidoResumo, type StatusPedido } from '../../types/pedido'
...
export function PedidosListaPage() {
  ...
  const [filtro, setFiltro] = useState<PedidoFiltro>({ pagina: 1, tamanhoPagina: 10 })
  const [filtrosTela, setFiltrosTela] = useState<FiltrosTela>(FILTROS_VAZIOS)
  const [filtroAberto, setFiltroAberto] = useState(false)

  const { data, isFetching, isError, error } = useListaPedidos(filtro)
  ...
  const abrirPedido = (pedido: PedidoResumo) => navegar(`/pedidos/${pedido.id}`)

  const colunaAcoes: NonNullable<TableProps<PedidoResumo>['columns']>[number] = {
    title: 'Ações',
    key: 'acoes',
    width: 88,
    align: 'center',
    render: (_, pedido) => (
      <Tooltip title="Abrir pedido">
        <Button type="text" icon={<ArrowRightOutlined />} aria-label={`Abrir pedido ${pedido.id}`} onClick={() => abrirPedido(pedido)} />
      </Tooltip>
    ),
  }

  // No celular, os dados de cada pedido ficam empilhados numa única coluna.
  const colunasCelular: TableProps<PedidoResumo>['columns'] = [
    {
      title: 'Pedido',
      key: 'pedido',
      render: (_, pedido) => (
        <div className="celula-compacta">
          ...
          <span className="celula-nome">{pedido.clienteNome}</span>
  ...
  const colunas: TableProps<PedidoResumo>['columns'] = [
    {
      title: 'Nº',
      dataIndex: 'id',
      width: 96,
  ...
  const conteudoFiltro = (
    <div className="popover-filtros">
      <div>
        <label className="rotulo-filtro" htmlFor="filtro-busca-pedido">
          <SearchOutlined /> Buscar pedido
        </label>
        <Input
          id="filtro-busca-pedido"
      ...
      <div>
        <label className="rotulo-filtro" htmlFor="filtro-status-pedido">
          Status
        </label>
        <Select
  ...
  return (
    <div className="pagina-pedidos">
      ...
      <section className="painel painel-tabela" aria-label="Lista de pedidos">
        <Table<PedidoResumo>
          rowKey="id"
          ...
          locale={{
            emptyText: quantidadeFiltros > 0 ? 'Nenhum pedido corresponde aos filtros.' : 'Nenhum pedido cadastrado.',
          }}
